...
- **Unverified Import**: `import_tir` always verifies; `load_tir(verify=False)` is disabled. Inspection without verification requires the explicit `load_tir_unverified()` call and must never write to a durable NodeLog.
- **Integrity vs. Provenance**: Node hash verification proves content integrity, not publisher authenticity. Package signatures remain reserved and unimplemented until a cryptography-focused design lands.
